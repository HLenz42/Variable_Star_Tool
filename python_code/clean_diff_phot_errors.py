...
target = sys.argv[2]
date = sys.argv[3]
input_path = sys.argv[1] + f"/tracked_stars_with_differential_mags_{target}_{date}.csv"
output_dir = sys.argv[1]
output_csv = f"tracked_stars_with_differential_mags_{target}_{date}.csv"
# output_csv = f"tracked_stars_with_updated_variability_flag_{target}_{date}.csv"
output_log = f"error_cleaning_log_{target}_{date}.txt"

# Column names 
TIME_COLUMN = 'julian_date'
STAR_INDEX_COLUMN = 'star_index'
UNWEIGHTED_MAG_COLUMN = 'differential_mag'
UNWEIGHTED_ERROR_COLUMN = 'differential_mag_err'
WEIGHTED_MAG_COLUMN = 'weighted_differential_mag'
WEIGHTED_ERROR_COLUMN = 'weighted_differential_mag_err'
...
if __name__ == "__main__":
    ...
    required_columns = [STAR_INDEX_COLUMN, TIME_COLUMN, UNWEIGHTED_MAG_COLUMN, UNWEIGHTED_ERROR_COLUMN, 
                        WEIGHTED_MAG_COLUMN, WEIGHTED_ERROR_COLUMN]
    ...
    print("\nError statistics:")
    unweighted_err = df[UNWEIGHTED_ERROR_COLUMN].dropna()
    weighted_err = df[WEIGHTED_ERROR_COLUMN].dropna()
    
    print(f"\nUnweighted differential magnitude error ({UNWEIGHTED_ERROR_COLUMN}):")
    print(f"  Maximum: {unweighted_err.max():.4f}")
    print(f"  Minimum: {unweighted_err.min():.4f}")
    print(f"  Average: {unweighted_err.mean():.4f}")
    
    print(f"\nWeighted differential magnitude error ({WEIGHTED_ERROR_COLUMN}):")
    print(f"  Maximum: {weighted_err.max():.4f}")
    print(f"  Minimum: {weighted_err.min():.4f}")
    print(f"  Average: {weighted_err.mean():.4f}")

    # Prompt user to choose error column for threshold
    while True:
        print("\n>>> Define error limit based on which column?")
        print("(1) Unweighted differential magnitude error")
        print("(2) Weighted differential magnitude error\n")
        which_err = int(input("Answer: "))
        print()

        if which_err == 1:
            ERROR_COLUMN = UNWEIGHTED_ERROR_COLUMN
            break
        if which_err == 2:
            ERROR_COLUMN = WEIGHTED_ERROR_COLUMN
            break
        else:
    ...
    # Initialize log
    log_lines = []
    log_lines.append("===== Differential Photometry Error Cleaning Summary =====")
    ...
    for star_idx in df[STAR_INDEX_COLUMN].unique():
        ...
        if num_high_errors > 0:
            ...
            # Set all four differential photometry columns to NaN
            df.loc[high_error_indices, [UNWEIGHTED_MAG_COLUMN, UNWEIGHTED_ERROR_COLUMN, 
                                        WEIGHTED_MAG_COLUMN, WEIGHTED_ERROR_COLUMN]] = np.nan
    ...
    # Save updated CSV, overwriting any existing file
